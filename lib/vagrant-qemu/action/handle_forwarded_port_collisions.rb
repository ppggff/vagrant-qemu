    ...
    module Action
      ...
      class HandleForwardedPortCollisions
        ...
        def handle(env)
          ...
          with_forwarded_ports(env) do |options|
            ...
            if in_use
              ...
              options[:ssh_port] = repaired_port
              env[:machine].provider_config.ssh_port = repaired_port
              puts("new port", repaired_port)
              # Notify the user
        ...
        def self.port_check(machine, host_ip, host_port)
          ...
          if Vagrant::Util::Platform.windows? && test_host_ip == "0.0.0.0"
